...
k.push(P('（三）**场地条件**：本项目实验室已建成并具备功能分区，功能分区自上游至下游依次为**试剂准备区、样本制备区（组织）、样本制备区（血浆）、破碎区（核酸片段化）、文库制备区、杂交捕获区、扩增区、测序区**，各区独立成间并按单向流程布置。中标人应在设备进场前完成现场勘察，就设备就位所需的台面承重、供电、给排水、温湿度与网络接入等条件提出书面要求；**因设备就位需对既有实验台面及配套设施进行局部适配调整的，由中标人负责实施，费用计入投标报价**。'));
...
k.push(P('（三）**报价方式**：试剂以**所对应检测项目的收费标准为基准**报价，收费标准按江苏省现行医疗服务价格政策执行。**投标人所报试剂单价，不高于所对应检测项目收费标准的 ＿＿ ％**，该比例在服务期内保持不变。**服务期内医疗服务价格政策调整的，按调整后的收费标准与所报比例执行。**所投试剂纳入国家或省级集中带量采购范围的，按中选结果执行。'));
...
k.push(P('（五）**移机服务**。服务期内因院方场地调整需要移机的，投标人须**单独报出移机服务价格**，作为可选报价项单独列示，由院方按实际需要决定是否实施；移机不影响原质保期的连续计算。'));
k.push(P('（六）**数据安全**。本平台产生的检测数据归院方所有。分析解读软硬件应支持院内本地化部署，**检测数据应存储于院内，不得上传至院外，不得出境**；涉及人类遗传资源的活动，应符合《中华人民共和国人类遗传资源管理条例》及其实施细则的规定。'));

k.push(new Paragraph({ children: runs('本文件为**采购需求**，仅描述本项目的技术与商务需求。投标人资格条件、评审办法、合同条款及其他未尽事宜，以招标文件相应章节及采购合同约定为准。', { size: 19 }),
      spacing: { before: 160, after: 120, line: 320 }, indent: { firstLine: IND },
      alignment: AlignmentType.BOTH }));

k.push(H2('附件一　配套设备与器具清单'));
k.push(P('本清单按功能分区列明，共 ' + D.total + ' 项、' + D.qty + ' 台（件），**由中标人全部供应**。清单以设备通用名称与技术参数描述需求，不指定品牌与型号。基因测序仪的要求见正文第二部分。'));
...
k.push(new Paragraph({ children: runs('注：上列为各癌种的**核心检测基因，属最低检测范围要求**。投标产品的检测范围可宽于上列内容，**覆盖更多靶点的产品不因此受限**；投标人应在投标文件中列明所投产品的完整检测范围。检测项目的收费按江苏省现行医疗服务价格政策执行，现行适用收费编码 012100000200000「高通量测序法检测费（病理样本）」，按项目分为 4,500 元、7,000 元、9,900 元等档次；**具体产品适用的档次与最终开展范围，以院方完成新增项目论证与物价备案后的结果为准。**', { size: 19 }),
