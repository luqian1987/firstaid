...
k.push(P('（五）**移机服务**。服务期内因院方场地调整需要将设备整体或部分移位的，投标人须**单独报出移机服务价格**，内容包括设备拆卸、包装、运输、复位、重新安装调试，以及配合完成移机后的方法学性能验证。该项报价作为可选报价项单独列示，不计入设备与试剂的评标价，中标后由院方按实际需要决定是否实施。**移机不影响原质保期的连续计算**，因移机导致的设备损坏或性能偏差由中标人负责修复。'));
k.push(P('（六）**设备所有权**。**本次采购设备的所有权自最终验收合格之日起归院方所有**，中标人不得设置任何形式的使用限制。'));
k.push(P('（七）**数据权属与信息安全**。本平台产生的全部检测数据、原始下机数据及衍生数据，**所有权归院方所有**。分析解读软硬件应支持院内本地化部署，不得要求将原始数据上传至院外。中标人及其人员因技术支持接触院方数据的，限于履行本合同之目的，不得留存、复制、对外提供或用于自身研发；涉及人类遗传资源的，应遵守《人类遗传资源管理条例》及其实施细则的规定。合同终止或人员撤场时，应完成数据交接与本地留存的清除并出具书面确认。'));
k.push(P('（八）**违约与退出**。合同应约定交付逾期、验收不合格、响应超时、供应中断的违约责任；服务期满或提前终止时，中标人应完成设备资料、系统账号、方法文件与在院数据的完整交接，不得影响检测工作的连续性。'));
