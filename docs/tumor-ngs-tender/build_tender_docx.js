...
k.push(P('（二）**采购范围**：基因测序仪 1 台、配套设备与器具（详见附件一）、拟开展检测项目所需的检测试剂（详见附件二），以及相应的安装调试、培训、性能验证配合与售后服务。**本项目为整体采购，上述内容应由同一投标人整体投标，不接受分包，不接受联合体投标。**'));
...
k.push(P('（六）**数据**。本平台产生的检测数据归院方所有；分析解读软硬件应支持院内本地化部署，不得要求将原始数据上传至院外。'));
