...
rooms=[]; total=0; qty=0
for R in m.D["rooms"]:
    rows=[[dev, m.GENERIC.get(dev,sp), n] for dev,sp,ref,n in R["rows"] if dev not in m.SEQ]
    if rows: rooms.append(dict(zone=R["zone"], rows=rows)); total+=len(rows); qty+=sum(r[2] for r in rows)
tech=[(t, d.replace('<b>','**').replace('</b>','**')) for t,d in m.TECH]
json.dump(dict(tech=tech, proj=m.PROJ, rooms=rooms, total=total, qty=qty),
