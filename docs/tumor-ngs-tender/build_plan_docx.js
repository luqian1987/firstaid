...
k.push(P('本项目为**整体采购**：基因测序仪、配套设备与器具、拟开展检测项目所需的检测试剂及相应服务，由同一投标人整体投标，**不接受分包，不接受联合体投标；所投产品应为本国产品**。实验室已建成并具备功能分区，自上游至下游依次为**试剂准备区、样本制备区（组织）、样本制备区（血浆）、破碎区（核酸片段化）、文库制备区、杂交捕获区、扩增区、测序区**，各区独立成间并按单向流程布置；本次不涉及新建与装修改造，设备就位所需的台面承重等局部适配由中标人负责。服务期限自合同签订之日起 ＿＿ 年。'));
...
const QUOTE = [
 ...
 ['维保', '质保期满后的维保价格一并报出。'],
 ['移机服务', '服务期内因院方场地调整需要移机的，**单独报出移机服务价格，作为可选报价项单独列示**，由院方按实际需要决定是否实施；移机不影响原质保期的连续计算。'],
];
...
k.push(new Paragraph({ children: runs('本文件为**建设需求**，仅描述本项目的技术与商务需求。投标人资格条件、评审办法、合同条款及其他未尽事宜，以招标文件相应章节及采购合同约定为准。', { size: 19 }),
      spacing: { before: 160, after: 120, line: 320 }, indent: { firstLine: IND },
      alignment: AlignmentType.BOTH }));

k.push(H2('附件一　设备清单'));
k.push(P('本清单按功能分区列明，共 ' + D.total + ' 项、' + D.qty + ' 台（件），**由中标人全部供应**。清单以设备通用名称与技术参数描述需求，**不指定品牌与型号**；投标人应逐项应答所投产品的品牌、型号与技术参数。基因测序仪的要求见表 1。'));
